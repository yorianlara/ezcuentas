...
@section('title') {{config('app.name')}} | Origin @endsection
@section('content')
@section('pagetitle') Origin @endsection
<!-- start page title -->
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                <h4 class="page-title">Origins</h4>
    ...
    <!-- end row -->
    <!-- Crear marcas -->
    <div class="modal fade" id="modalOrigins" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form  class="needs-validation" novalidate id="frmOrigins">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title" id="titulo">Origin</h5>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('#modalOrigins').on('hide.bs.modal',function(){
            $('#id').val(null);
            $('#name').val(null);
            ...
            $('#frmOrigins').removeClass('was-validated');
        });

        $('#cerrar').on('click',function(){
            $('#modalOrigins').modal('hide');
        });

        $('#frmOrigins').on('submit',function(e){
            e.preventDefault();
            ...
            $.post('{{ route("origin.save")}}',{"id":id,"name":name,"status":status,"_token":token})
            .done(function(data){
    ...
    async function eliminar(id) {
        ...
        {
            let token = "{{ csrf_token() }}";
            $.post('{{ route("origin.delete")}}',{"id":id,"_token":token})
            .done(function(data){
                showToast(data.msg,data.success);
